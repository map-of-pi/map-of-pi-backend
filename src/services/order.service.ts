...
import { getRollbackStockLevel, getUpdatedStockLevel } from "../helpers/order";
...
export const createOrder = async (orderData: NewOrder): Promise<IOrder> => {
  ...
  try {
    ...
    const sellerItems = await SellerItem.find({
      _id: { $in: sellerItemIds },
    }).session(session);
    const sellerItemMap = new Map(
      sellerItems.map((doc) => [doc._id.toString(), doc])
    );
    ...
    for (const item of orderData.orderItems) {
      ...
      const newStockLevel = getUpdatedStockLevel(
    ...
    logger.info("Order and stock levels created/updated successfully", {
    ...
    if (error instanceof StockValidationError) {
      logger.warn(`Stock validation failed: ${error.message}`, {
        itemId: error.itemId,
      });
    } else {
...
export const updatePaidOrder = async (paymentId: string): Promise<IOrder> => {
  try {
    ...
    logger.error(
      `Failed to update paid order for paymentID ${paymentId}: ${error}`
    );
    throw error;
  }
};
...
export const getOrderItems = async (orderId: string) => {
  try {
    ...
    logger.info(
      `Fetched ${orderItems.length} order items for orderID ${orderId}`
    );
  ...
  try {
    ...
    if (orderStatus === OrderStatusType.Completed) {
      ...
      if (orderItems.length > 0) {
        ...
        logger.info(
          `Marked ${orderItems.length} order items as fulfilled for orderID ${orderId}`
        );
      } else {
        logger.warn(
          `No order items found to mark as fulfilled for orderID ${orderId}`
        );
      }
    } else {
      logger.warn(
        `Unhandled order status type: ${orderStatus} for orderID ${orderId}`
      );
    }
    ...
    // Send notification to buyer when seller completes the order
    if (
      orderStatus.toLowerCase() === OrderStatusType.Completed.toLowerCase() &&
      updatedOrder
    ) {
      try {
        // Get seller and buyer info
        const seller = await Seller.findById(updatedOrder.seller_id);
        const buyer = await User.findById(updatedOrder.buyer_id);

        if (seller && buyer) {
          const message = `The seller ${
            seller.seller_id
          } fulfilled your order on ${new Date().toLocaleString()} and it's ready for collection/delivery.`;

          // Use the existing notification API
          await addNotification(buyer.pi_uid, message);

          logger.info(
            `Notification sent to buyer ${buyer.pi_uid} for order ${orderId}`
          );
        } else {
          logger.warn(
            `Seller or buyer not found when sending notification for order ${orderId}`
          );
        }
      } catch (error) {
        logger.error(
          ` Error sending notification for order ${orderId}: ${error}`
        );
      }
    }
    ...
    if (!updatedOrder) {
      logger.error(
        `Order not found or failed to update status for orderID ${orderId}`
      );
      return null;
    }
    ...
    logger.error(
      `Failed to update order status for orderID ${orderId}: ${error}`
    );
    throw error;
  }
};
...
export const updateOrderItemStatus = async (
  ...
  try {
    ...
    if (!updatedItem) {
      logger.error(
        `Order item not found or failed to update for itemId: ${itemId}`
      );
      return null;
    }
    ...
    logger.error(
      `Failed to update order item status for orderItemID ${itemId}: ${error}`
    );
    throw error;
  }
};
...
export const cancelOrder = async (paymentId: string) => {
  ...
  try {
    ...
    if (orderItems.length === 0) {
      logger.warn(
        `No pending order items found for order ${cancelledOrder._id}`
      );
    }
    ...
    const sellerItemMap = new Map(
      sellerItems.map((item) => [item._id.toString(), item])
    );
    ...
    for (const item of orderItems) {
      ...
      const restoredLevel = getRollbackStockLevel(
        sellerItem.stock_level,
        item.quantity
      );
    ...
    await session.commitTransaction();
    logger.info(
      `Order with paymentID ${paymentId} successfully cancelled and stock levels restored.`
    );
    return cancelledOrder;
  } catch (error: any) {
    await session.abortTransaction();
    logger.error(
      `Failed to cancel order for paymentID ${paymentId}: ${error.message}`
    );
    throw error;
  } finally {
    session.endSession();
